package io.github.sofistico.hex_open_book.registry
...
typealias RegistrarEntry<T> = HexOpenBookRegistrar<T>.Entry<out T>

abstract class HexOpenBookRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(HexOpenBook.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexOpenBookRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
